---
title: "Cell Ranger Mapping Metrics QC"
author: "Esther Zhou"
date: "2026-04-03"
output: html_document
---
#Import Mapping Metrics
```{r}
...
df$sample_short <- sub("^(M[0-9]+_D[0-9]+).*", "\\1", df$sample_folder)
...
#Percent reads mapped to mouse genome
```{r}
png("Percent_Reads_Mapped_Mouse_Genome.png", width = 1800, height = 1400, res = 300)
...
#Percent reads confidently mapped to mouse genome
```{r}
...
min_idx <- which.min(df$mapped_conf)

text(df$x[min_idx], df$mapped_conf[min_idx],
     labels = df$sample_short[min_idx],
     pos = 3,
     cex = 0.8)
