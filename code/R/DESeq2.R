...
#a variance-stabilizing transformation that also accounts for library size (normalization)
...
ggsave(
  filename = "/work/users/x/c/xczhou/Chlamydia/Analysis Outputs/PCA_plot.png",
  plot = p_pca,
  width = 6,
  height = 5,
  dpi = 300
...
ggsave(paste0("/work/users/x/c/xczhou/Chlamydia/Analysis Outputs/heatmap_all_samples.tiff"),
       heat1_annotation,
       device = "tiff",
       width =12,
       dpi = 300)
...
p1 <- ggplot(PCA_data,
  ...
  theme(axis.text.x = element_text(), #angle = 30
        ...
        legend.position = c(1, 0.01),  # Bottom-right corner
...
ggsave(paste0("/work/users/x/c/xczhou/Chlamydia/Analysis Outputs/PCA_plot.tiff"),
       p1,
       device = "tiff",
       width =12,
       dpi = 300)
...
plot_DE <- function(All.DE, name){
  ...
  ggsave(paste0("/work/users/x/c/xczhou/Chlamydia/Analysis Outputs/",name,"_DE.png"), plot = DEplot, dpi = 300, width = 5.3, height = 5)
  # width = 4.3, height = 5
...
#DE Analysis Day4
dds4 <- dds_mouse[, dds_mouse$day==4]
...
write.csv(res_vaccine_naive_day4_table%>%
          ...
          file = "/work/users/x/c/xczhou/Chlamydia/Analysis Outputs/Day4_up_DE_p.csv")

write.csv(res_vaccine_naive_day4_table%>%
            arrange(log2FoldChange)%>%
            filter(pvalue<0.05 & log2FoldChange < 0), 
          file = "/work/users/x/c/xczhou/Chlamydia/Analysis Outputs/Day4_down_DE_p.csv")
...
write.csv(res_vaccine_naive_day4_table%>%
          ...
          file = "/work/users/x/c/xczhou/Chlamydia/Analysis Outputs/Day4_up_DE_padj.csv")

write.csv(res_vaccine_naive_day4_table%>%
            arrange(log2FoldChange)%>%
            filter(padj<0.05 & log2FoldChange < 0), 
          file = "/work/users/x/c/xczhou/Chlamydia/Analysis Outputs//Day4_down_DE_padj.csv")
